...
class MySubAddAction (M : Type*) (X : Type*) (Y : Set X) [AddMonoid M] [add_action_orig : AddAction M X] where
  ...
  SubAction_eq_Action : ∀ (c : M) (y : Y), ((c +ᵥ y):X) = c +ᵥ (y:X)

/-- The sub-action packaged in MySubAddAction as a type-class
instance, so that typeclass inference can find AddAction M Y. -/
instance MySubAddAction.toAddAction
    {M X : Type*} {Y : Set X}
    [AddMonoid M] [AddAction M X] [h : MySubAddAction M X Y] :
  AddAction M Y :=
  h.SubAction
  ...
  StronglyRecurrent (M := M) (X := X) (y : X) := by {
    ...
    have hsubset :
      ...
      have h2 : ↑(c +ᵥ (m +ᵥ y) : Y) = c +ᵥ ((m +ᵥ y):X) :=
        MySubAddAction.SubAction_eq_Action c (m +ᵥ y)
      rw [h1] at h2
...
class ContinuousAddAction (M X : Type*)
  [AddMonoid M] [TopologicalSpace X]
  extends AddAction M X where
  cont_smul : ∀ m : M,
    Continuous (fun x : X => m +ᵥ x)

axiom exists_minimal_invariant_subset
  {X : Type*} [TopologicalSpace X] [CompactSpace X] [Nonempty X]
  {M : Type*} [AddMonoid M] [ContinuousAddAction M X]:
